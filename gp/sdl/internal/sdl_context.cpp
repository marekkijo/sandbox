...
#ifdef __EMSCRIPTEN__
  emscripten_set_main_loop_arg(emscripten_main_loop, (void *)this, 0, true);
  return return_code_;
#else
  auto return_code = 0;
  auto quit_flag = false;
  while (!quit_flag) {
    return_code = exec_loop(quit_flag);
  }
  return return_code;
#endif
}
...
void SDLContext::emscripten_main_loop(void *arg) {
  ...
  auto quit_flag = false;
  context->return_code_ = context->exec_loop(quit_flag);
  if (quit_flag) {
...
int SDLContext::exec_loop(bool &quit_flag) const {
  int return_code = 0;
  SDL_Event sdl_event;
