...
void Encoder::flip_frame() {
  ...
  for (std::size_t i = 0; i < height; i++) {
    for (std::size_t j = 0; j < width; j++) {
      auto ptr_gl  = gl_frame_->data() + (channels_ * (width * (height - i - 1) + j));
      auto ptr_rgb = rgb_frame_.data() + (channels_ * (width * i + j));
      memcpy(ptr_rgb, ptr_gl, channels_);
    }
  }
}

void Encoder::rgb_to_yuv() {
  const int in_linesize[1] = {channels_ * context_->width};
